...
export class FormatProvider implements vscode.DocumentFormattingEditProvider {
    public provideDocumentFormattingEdits(document: vscode.TextDocument, options: vscode.FormattingOptions, token: vscode.CancellationToken): vscode.ProviderResult<vscode.TextEdit[]> {
        let opts = {"indent_size":"1","indent_char":"    ","max_preserve_newlines":"2","preserve_newlines":true,"keep_array_indentation":true,"break_chained_methods":false,"indent_scripts":"keep","brace_style":"collapse","space_before_conditional":true,"unescape_strings":false,"wrap_line_length":"0","space_after_anon_function":true,"jslint_happy":true};
...
class Beautifier {
    ...
    constructor(js_source_text : string, options : any) {
        ...
        let token_text: string, token_type: string, last_type: string, last_last_text: string, indent_string: string;
        ...
        this.beautify = function () {
            ...
            while (true) {
                ...
                if (token_type !== 'TK_INLINE_COMMENT' && token_type !== 'TK_COMMENT' &&
                    ...
                    if (!following_bracket && token_type === 'TK_RESERVED' && in_array(token_text, ['if', 'for', 'while', 'loop'])) {
                        output_space_before_token = true;
                        following_bracket = true;
                        bracketnum = 0;
                        if (in_array(flags.last_text, ['if', 'while'])) {
        ...
        function start_of_statement() {
            if ((last_type === 'TK_RESERVED' && in_array(flags.last_text, ['local', 'static', 'global']) && token_type === 'TK_WORD') ||
                ...
                (last_type === 'TK_RESERVED' && flags.last_text.match(/^else$/i) && !(token_type === 'TK_RESERVED' && token_text.match(/^if$/i))) ||
                (previous_flags.mode === MODE.ForInitializer || previous_flags.mode === MODE.Conditional) ||
                // (last_type === 'TK_END_EXPR' && (previous_flags.mode === MODE.ForInitializer || previous_flags.mode === MODE.Conditional)) ||
                (last_type === 'TK_WORD' && flags.mode === MODE.BlockStatement
                    && !flags.in_case && !(token_text === '--' || token_text === '++')
                ...
                if (!start_of_object_property()) {
                    allow_wrap_or_preserved_newline(token_type === 'TK_RESERVED' && in_array(token_text, ['loop', 'for', 'if', 'while']));
                }
        ...
        function end_bracket_of_expression(pos : number) {
            ...
            if (!(LF.length === 0|| bracketnum > 0 || LF.match(/^([;#]|\/\*|(and|or|is|in)\b)/i) || in_array(LF.charAt(0), punct))) {
                following_bracket = false;
                restore_mode();
                remove_redundant_indentation(previous_flags);
        ...
        function get_next_token() {
            ...
            if (in_array(c, punct)) {
                ...
                if (c === ',') {
                    ...
                    let LF = input.substring(f, pLF).trimRight();
                    let ph = LF.indexOf('::', parser_pos);
                    if (ph > -1) {
                        parser_pos = f + ph;
                        print_newline();
                        let p=input.substring(f - 1, parser_pos)
                        return [input.substring(f - 1, parser_pos), 'TK_WORD'];
                    }
                    return [c, 'TK_OPERATOR'];
                }
            }
        ...
        function handle_word() {
            ...
            if (flags.if_block) {
                if (!flags.else_block && (token_type === 'TK_RESERVED' && token_text.match(/^else$/i))) {
            ...
            if (token_type === 'TK_RESERVED' && (token_text.match(/^case$/i) || (token_text.match(/^default$/i) && flags.in_case_statement))) {
                print_newline();
            ...
            if (last_type === 'TK_END_BLOCK') {
                if (!(token_type === 'TK_RESERVED' && in_array(token_text, ['else', 'until', 'catch', 'finally']))) {
                    prefix = 'NEWLINE';
                } else {
            ...
            if (token_type === 'TK_RESERVED' && in_array(token_text, ['else', 'until', 'catch', 'finally'])) {
                ...
                if (last_type === 'TK_RESERVED' && is_special_word(flags.last_text)) {
                    ...
                    if ((last_type !== 'TK_START_EXPR' || !(token_type === 'TK_RESERVED' && in_array(token_text, ['local', 'static', 'global']))) && flags.last_text !== ':') {
                        // no need to force newline on 'let': for (let x = 0...)
                        if (token_type === 'TK_RESERVED' && token_text.match(/^if$/i) && flags.last_word.match(/^else$/i) && flags.last_text !== '{') {
            ...
            if (token_type === 'TK_RESERVED' && token_text.match(/^loop$/i)) {
                flags.do_block = true;
            }

            if (token_type === 'TK_RESERVED' && token_text.match(/^if$/i)) {
                flags.if_block = true;
            }
        }
